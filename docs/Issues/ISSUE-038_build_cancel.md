...
func CancelRunningBuild(ctx context.Context, db *gorm.DB, k8sClient *kubernetes.Clientset, deploymentID string) error {
    ...
    err := db.Where("deployment_id = ? AND status = ?", deploymentID, model.BuildStatusBuilding).
        First(&build).Error
    ...
    db.Model(&build).Update("status", model.BuildStatusCancelled)
