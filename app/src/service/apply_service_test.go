...
func TestApplyService_Apply_正常にapplyされk8sDeploymentが作成される(t *testing.T) {
	...
	applyService := NewApplyService(db, fakeK8sClient, nil, deploymentRepo, applyHistoryRepo, projectRepo, serviceRepo, ingressRouteRepo, repository.NewEnvVarRepository(db), repository.NewEnvVarMountRepository(db)) // サービスを生成する
...
func TestApplyService_Apply_k8sapply失敗時にapplyHistorystatusがfailedになる(t *testing.T) {
	...
	applyService := NewApplyService(db, fakeK8sClient, nil, deploymentMock, applyHistoryMock, projectMock, repository.NewServiceRepository(db), repository.NewIngressRouteRepository(db), repository.NewEnvVarRepository(db), repository.NewEnvVarMountRepository(db)) // サービスを生成する
...
func TestApplyService_Apply_k8sapply失敗時にpendingフィールドがそのまま残る(t *testing.T) {
	...
	deploymentRepo := repository.NewDeploymentRepository(db)     // リポジトリを生成する
	applyHistoryRepo := repository.NewApplyHistoryRepository(db) // apply_history リポジトリを生成する
	applyService := NewApplyService(db, fakeK8sClient, nil, deploymentRepo, applyHistoryRepo, projectMock, repository.NewServiceRepository(db), repository.NewIngressRouteRepository(db), repository.NewEnvVarRepository(db), repository.NewEnvVarMountRepository(db)) // サービスを生成する
...
func TestApplyService_ListApplyHistories_正常に履歴一覧が取得できる(t *testing.T) {
	...
	projectRepo := repository.NewProjectRepository(db)                // project リポジトリを生成する
	applyService := NewApplyService(db, nil, nil, deploymentRepo, applyHistoryRepo, projectRepo, repository.NewServiceRepository(db), repository.NewIngressRouteRepository(db), repository.NewEnvVarRepository(db), repository.NewEnvVarMountRepository(db)) // サービスを生成する
...
func TestApplyService_Apply_applyでConfigMapとSecretが作成される(t *testing.T) {
	...
	applyService := NewApplyService(db, fakeK8sClient, nil, deploymentRepo, applyHistoryRepo, projectRepo, serviceRepo, ingressRouteRepo, envVarRepo, envVarMountRepo) // サービスを生成する
...
func TestApplyService_Apply_重複キーが存在する場合applyがエラーになる(t *testing.T) {
	...
	fakeK8sClient := fake.NewSimpleClientset()                       // fake k8s クライアントを生成する
	applyService := NewApplyService(db, fakeK8sClient, nil, deploymentMock, applyHistoryMockRepo, projectMock, repository.NewServiceRepository(db), repository.NewIngressRouteRepository(db), envVarMock, envVarMountMock) // サービスを生成する
