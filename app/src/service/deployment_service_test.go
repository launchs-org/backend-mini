...
func TestCreateDeployment_正常に作成されpendingフィールドに値が入る(t *testing.T) {
	...
	deploymentSvc := NewDeploymentService(deploymentRepo, serviceRepo) // サービスを生成する
...
func TestUpdateDeployment_送ったフィールドのみpendingが更新される(t *testing.T) {
	...
	_, err := deploymentSvc.UpdateDeployment(context.Background(), "deployment-id-1", req) // サービスを実行する
	if err != nil {
...
func TestUpdateDeployment_存在しないdeploymentはエラーを返す(t *testing.T) {
	...
	_, err := deploymentSvc.UpdateDeployment(context.Background(), "nonexistent", req) // サービスを実行する
	if err == nil { // エラーが返ることを確認する
		t.Fatal("エラーが返るべきですが nil が返りました")
	}
}
...
func TestDeleteDeployment_statusがdeletingになる(t *testing.T) {
	...
	result, err := deploymentSvc.DeleteDeployment(context.Background(), "deployment-id-1") // サービスを実行する
	if err != nil {
...
func TestDeleteDeployment_存在しないdeploymentはエラーを返す(t *testing.T) {
	...
	_, err := deploymentSvc.DeleteDeployment(context.Background(), "nonexistent") // サービスを実行する
	if err == nil { // エラーが返ることを確認する
		t.Fatal("エラーが返るべきですが nil が返りました")
	}
}
